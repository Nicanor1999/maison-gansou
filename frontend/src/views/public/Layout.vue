          ...
          <nav v-if="showServicesNav" class="services-nav-container">
            <ul class="flex gap-2 md:gap-4 overflow-x-auto whitespace-nowrap hide-scrollbar items-center text-gray-600 text-xs md:text-sm">
              <li v-for="item in servicesNavItems" :key="item.path" class="min-w-max">
                <router-link
                  :to="item.path"
                  class="px-2 py-2 text-center hover:text-[var(--second-orange)] transition-colors duration-300"
...
export default {
  ...
  setup() {
    ...
    const showServicesNav = computed(() => {
      console.log('showServicesNav computed:', uiStore.showServicesNav)
      return uiStore.showServicesNav
    })
